...
<head>
    ...
    <title>Reviews: {{ $hotel->name}} </title>
</head>
<body>
...
@if(auth()->user()?->is_admin || auth()->user()?->id === $review->user_id)
    <form action = "{{ route('review_hotel_delete', $review->id) }}" method = "POST">
    @csrf
    @method('DELETE')
...
<form action = "{{ route('review_hotel_store', ['hotel_id' => $hotel->id] )}}" method = "POST" class="form">
        @csrf
